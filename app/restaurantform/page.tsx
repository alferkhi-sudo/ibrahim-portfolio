...
function Step2({ answers, set }: { answers: Answers; set: (k: string, v: string | string[]) => void }) {
  return (
    <>
      ...
      <Question num={9} label="Moment préféré pour sortir">
      ...
      <Question num={10} label="Distance acceptable pour un bon restaurant">
      ...
      <Question num={11} label="Recourez-vous à la livraison à domicile ?">
...
function Step3({ answers, set }: { answers: Answers; set: (k: string, v: string | string[]) => void }) {
  return (
    <>
      <Question num={12} label="Comment évaluez-vous l'offre de restauration dans votre zone ?">
      ...
      <Question num={13} label="Quels types d'établissements trouvez-vous trop nombreux ?">
      ...
      <Question num={14} label="Qu'est-ce qui manque dans votre zone ?">
...
function Step4({ answers, set }: { answers: Answers; set: (k: string, v: string | string[]) => void }) {
  return (
    <>
      <Question num={19} label="Quels sont vos 3 critères les plus importants ?">
      ...
      <Question num={20} label="Ce qui vous déçoit le plus actuellement dans la restauration locale">
        <Textarea name="q22" value={answers.q22 as string ?? ""} onChange={(v) => set("q22", v)} placeholder="Vos impressions..." />
      </Question>
      <Question num={21} label="La musique influence-t-elle votre expérience au restaurant ?">
      ...
      <Question num={22} label="Êtes-vous sensible au parcours authentique du cuisinier ?">
      ...
      <Question num={23} label="Préférez-vous une carte courte ou longue ?">
        <Radio options={["Carte longue", "Carte courte, gage de fraîcheur", "Peu importe"]} value={answers.q25 as string ?? ""} onChange={(v) => set("q25", v)} />
      </Question>
      <Question num={24} label="L'identité visuelle d'un restaurant influence-t-elle votre choix avant d'y aller ?">
...
function Step6({ answers, set }: { answers: Answers; set: (k: string, v: string | string[]) => void }) {
  return (
    <>
      <Question num={32} label="Comment découvrez-vous de nouveaux restaurants ?">
      ...
      <Question num={33} label="Ce qui vous inciterait à revenir régulièrement">
      ...
      <Question num={34} label="Laissez-vous des avis en ligne ?">
        <Radio options={["Non, jamais", "Rarement", "Parfois", "Oui, régulièrement"]} value={answers.q38 as string ?? ""} onChange={(v) => set("q38", v)} />
      </Question>
      <Question num={35} label="Commentaire libre — vos attentes ou remarques pour ce projet">
